...
const actions = {
  ...
  berserk: {
    ...
    cast: 0,
    recast: 60,
    description: `Increases attack power by 30%.`,
    execute(state) {
      setStatus("berserk", true);
    }
  },
  defiance: {
    name: "Defiance",
    ...
    execute(state) {
      if(hasStatus("defiance")) {
        ...
        if(hasStatus("unchained")) {
          setStatus("unchained", false);
        }
      }else if(hasStatus("deliverance")) {
        setStatus("deliverance", false);
        setStatus("defiance", true);
        setBeast(Math.floor(state.beast / 2));
      }else {
        setStatus("defiance", true);
      }
    }
    ...
    execute(state) {
      if(hasStatus("deliverance")) {
        ...
        if(hasStatus("innerrelease")) {
          setStatus("innerrelease", false);
        }
      }else if(hasStatus("defiance")) {
        setStatus("defiance", false);
        setStatus("deliverance", true);
        setBeast(Math.floor(state.beast / 2));
      }else {
        setStatus("deliverance", true);
      }
  ...
  infuriate: {
    ...
    useable(state) {
      return (hasStatus("deliverance") || hasStatus("defiance"));
    }
  },
  innerrelease: {
    name: "Inner Release",
...
const statuses = {
  ...
  unchained: {
    name: "Unchained",
    duration: 20,
    description: "dammy sentence"
  },
  innerrelease: {
    name: "InnerRelease",
    duration: 20,
    description: "lighting eyes"
  },
  berserk: {
    name: "Berserk",
    duration: 20,
